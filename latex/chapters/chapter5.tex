...
\bottomrule
\end{tabular}
\caption{Control plane resource requirements by deployment configuration. HA configurations assume replicated control plane instances with a load balancer. Proposed System HA figures assume three stateless replicas (0.5 vCPU / 1 GB each) behind a lightweight load balancer ($\sim$0.25 vCPU / 256 MB). Kubernetes data from \cite{k8s_docs}, Nomad data from \cite{nomad_docs}.}
\label{tab:resource-comparison}
\end{table}

Even in a comparable HA configuration (three stateless control plane replicas behind a load balancer), the proposed system requires significantly fewer resources than both Kubernetes and Nomad. This advantage stems from the stateless control plane design: because the control plane holds no in-memory cluster state, replicas are interchangeable and require no consensus protocol or leader election among themselves. Benchmarks of lightweight Kubernetes distributions on edge hardware \cite{cilic2023edgeeval, usman2024lightweight} report similar overhead patterns, confirming that eliminating the etcd dependency and scheduler loop yields measurable resource advantages even when HA replication is factored in.
